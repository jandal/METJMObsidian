...
const color = pcFile?.ability_colors?.intelligence || "#06A77D";

const skillValue = pcFile?.skill_arcana || 0;
const displayValue = skillValue >= 0 ? `+${skillValue}` : skillValue;

dv.paragraph(`
<div style="font-size: 40px; font-weight: bold; color: ${color}; text-align: center;">
  ${displayValue}
</div>
`)
```
